"""NQ 10y 8:30 ET news IFVG runner — 17 events × 3 exit variants × 2-way ES SMT × 3 sides × 11 year buckets.

Loads NQ + ES parquet ONCE for the full 10y window (2016-01-04 → 2026-05-11), then loops
over 17 event types. Output schema matches the existing site Explorer (same as ES/GC runners).

NQ-specific notes:
- Tick size: NQ = 0.25 pt (ES = 0.25 pt, GC = 0.10 pt). simulate_variant and aggregate_rows
  are reused unmodified — they only care about entry/sl/tp prices, not tick size.
- NQ parquet: continuous ~3.6M bars, ts in America/New_York ns.
- ES parquet (sister): forward-filled continuous ~3.5M bars, ts in America/New_York ns.
- SMT semantics: NQ SHORT (sweep UP) → ES low < ES pre_low; NQ LONG (sweep DOWN) → ES high > ES pre_high.

Usage:
    /Users/angelo/monfxreplay-python/.venv/bin/python run_nq_10y_all_events.py
"""
...
def load_es_bars() -> dict[int, dict]:
    """Load ES parquet (SMT partner) → bar dict keyed by unix seconds (int)."""
...
def load_nq_bars() -> dict[int, dict]:
    """Load NQ parquet (primary) → bar dict keyed by unix seconds (int)."""
...
def detect_setup_nq(ev: dict, bars_idx: dict[int, dict]) -> dict:
    ...
    # Placeholder here to keep variable defined until matched_fvg / entry_ts known. NQ_TICK = 0.25.
    sl = None
...
# ---------------------------------------------------------------------------
# ES SMT detection (sister index for NQ)
# ---------------------------------------------------------------------------

def compute_es_smt(ev: dict, es_idx: dict[int, dict], side: str) -> bool:
    """Did ES take its target during release→sweep_deadline window?
    NQ SHORT (NQ swept UP) → ES low < ES pre_low.
    NQ LONG  (NQ swept DOWN) → ES high > ES pre_high.
    """
    event_type = ev["event_type"]
    t0 = ev["t0_utc"]
    et_date = ev["et_date"]
...
def collect_trade_db(event_type: str, nq_idx: dict, es_idx: dict) -> dict:
...
def main():
    ...
    print(f"[setup] loading NQ + ES bars once ({DATE_FROM}..{DATE_TO})...", file=sys.stderr)
    nq_idx = load_nq_bars()
    es_idx = load_es_bars()
    ...
    for event_type in EVENTS:
        ...
        res = collect_trade_db(event_type, nq_idx, es_idx)
    ...
    for event_type, evr in per_event_results.items():
        ...
        ev_out = {
            "meta": {
                "source": (
                    "NQ (Nasdaq 100 futures, winning-day-algo, 10y, 2016-01-04…2026-05-11)"
                    " + ES (SMT partner)"
                ),
                "date_from": DATE_FROM,
                "date_to":   DATE_TO,
                ...
                "tick": "NQ_TICK=0.25 (Nasdaq 100 futures)",
            },
            "rows":   evr["rows"],
            "trades": evr["trades"],
        }
        ...
        prices_out = {
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "source":       "run_nq_10y_all_events.py",
            "event":        event_type,
            "asset":        "NQ",
    ...
    out_path = "/Users/angelo/jacktradesnq-site-costs/public/data/nq-ifvg-smt.json"
    ...
    print(f"\n=== no_be / BOTH / ALL / smt=true (target) — NQ 10y ALL 17 EVENTS ===")
    if r_headline:
