...
// Affiliate / discount code with FundedSeat.
// Code "JTNQ" = exclusive discount for the buyer + 15% commission for JackTradesNQ.
const CODE = 'JTNQ';

// Referral link straight to FundedSeat with the JTNQ attribution + coupon auto-applied
// (verified live on the FundedSeat affiliate dashboard, 2026-06-29).
const AFFILIATE_URL = 'https://fundedseat.link/jtnq';
...
const FS = {
  ...
  legal: {
    ...
    disclaimer:
      'Disclaimer — In accordance with article D.321-1 of the French Monétaire et Financier Code, the content published on this site is provided for informational purposes only and does not constitute investment advice. This page is an advertisement: JackTradesNQ is an affiliate of FundedSeat and earns a commission on sign-ups made with code JTNQ.',
  },
};

export default function FundedSeat() {
  ...
  return (
    <div className="jtnq-fs">
      <style>{CSS}</style>

      <main>
        <section className="hero">
          <div className="back-bar">
            <a className="back" href={FS.backToHome.url}>
              ...
              <span>{FS.backToHome.label}</span>
            </a>
            <a className="back-join" href={FS.closer.cta.url} target="_blank" rel="noopener nofollow sponsored">
              Get funded
          ...
          <div className="eyebrow">{FS.hero.eyebrow}</div>
          <h1 className="wordmark">
            {FS.hero.title}
            <span className="dot">.</span>
          </h1>
          <p className="phrase">{FS.hero.phrase}</p>
        </section>
        ...
        <section className="firm">
          <div className="section-head">
            <h2>Why FundedSeat</h2>
            <span className="count">1-step &amp; instant · futures</span>
          </div>
        ...
        <section className="closer">
          <p className="lead">{FS.closer.lead}</p>
      ...
      <footer className="site-footer">
        <div className="footer-inner">
          ...
          <div className="footer-meta">
            <a href={FS.legal.mentionsUrl}>Legal</a>
            <span className="sep">·</span>
            <a href={FS.legal.privacyUrl}>Privacy policy</a>
            <br />
            <span>{FS.legal.copyright}</span>
          </div>
        </div>
        <p className="footer-disclaimer">{FS.legal.disclaimer}</p>
      </footer>
    </div>
  );
}
...
.jtnq-fs a{ color: inherit; text-decoration: none; }
.jtnq-fs ::selection{ background: var(--c-accent); color: var(--c-bg); }
...
.jtnq-fs .back .arr{ width: 14px; height: 14px; }
.jtnq-fs .back:hover{ color: var(--c-accent); gap: 12px; }
.jtnq-fs .back-join{
...
.jtnq-fs .back-join svg{ width: 11px; height: 11px; }
.jtnq-fs .back-join:hover{ background: var(--c-accent); color: var(--c-bg); transform: translateY(-1px); }
...
.jtnq-fs .firm-card .k::before{ content: ""; width: 6px; height: 6px; border-radius: 50%; background: var(--c-accent); }
.jtnq-fs .firm-card .v{
...
.jtnq-fs .plans-link svg{ width: 13px; height: 13px; }
.jtnq-fs .plans-link:hover{ color: var(--c-accent); gap: 13px; }
...
.jtnq-fs .code-row:hover{ border-color: var(--c-accent); transform: translateY(-1px); }
.jtnq-fs .code-row:focus-visible{ outline: 2px solid var(--c-accent); outline-offset: 3px; }
.jtnq-fs .code-row .code{
  font-family: var(--f-mono); font-weight: 500; font-size: clamp(28px, 4.5vw, 40px);
  letter-spacing: .14em; color: var(--c-text); line-height: 1;
}
.jtnq-fs .code-row .copy{
  flex-shrink: 0; display: inline-flex; align-items: center; gap: 7px;
  font-family: var(--f-mono); font-size: 11px; letter-spacing: .16em; text-transform: uppercase; color: var(--c-accent);
}
.jtnq-fs .code-row .copy svg{ width: 14px; height: 14px; }
.jtnq-fs .code-note{
...
.jtnq-fs .cta:hover{ transform: translateY(-2px); gap: 18px; background: oklch(0.83 0.14 78); }
.jtnq-fs .cta svg{ width: 16px; height: 16px; }
.jtnq-fs .note{
...
.jtnq-fs .note .gold{ color: var(--c-accent); margin: 0 8px; }

.jtnq-fs .site-footer{
  background: var(--c-bg-deep); border-top: 1px solid var(--c-line-soft); padding: 56px var(--pad-x) 48px;
}
.jtnq-fs .footer-inner{
  max-width: var(--maxw); margin: 0 auto; display: grid; grid-template-columns: 1fr auto; gap: 32px; align-items: start;
}
@media (max-width: 720px){ .jtnq-fs .footer-inner{ grid-template-columns: 1fr; } }
.jtnq-fs .footer-brand{ font-family: var(--f-serif); font-style: italic; font-size: 22px; color: var(--c-text); }
.jtnq-fs .footer-brand .dot{ color: var(--c-accent); }
.jtnq-fs .footer-meta{
  font-family: var(--f-mono); font-size: 11px; letter-spacing: .18em; text-transform: uppercase;
  color: var(--c-text-mute); text-align: right; line-height: 1.9;
}
@media (max-width: 720px){ .jtnq-fs .footer-meta{ text-align: left; } }
.jtnq-fs .footer-meta a{ color: var(--c-text-soft); }
.jtnq-fs .footer-meta a:hover{ color: var(--c-accent); }
.jtnq-fs .footer-meta .sep{ color: var(--c-text-deep); margin: 0 10px; }
.jtnq-fs .footer-disclaimer{
